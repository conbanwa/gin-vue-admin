  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="类型" prop="type" width="120">
            ...
            {{ filterDict(scope.row.type,gardenTypeOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="简介" prop="spec" width="120" />
        <el-table-column align="left" label="详情" prop="power" width="120" />
        <el-table-column align="left" label="图片" prop="pic" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateProductionFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getProductionList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteProductionByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateProductionFunc = async(row) => {
    const res = await findProduction({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteProductionFunc = async (row) => {
    const res = await deleteProduction({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
